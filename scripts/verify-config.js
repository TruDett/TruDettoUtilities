		...
		return new Promise((res) => {
			...
			client.login(config.token).then(async () => {
				...
				if(!client.guilds.cache.has("568120814776614924")){
					error("Error, the bot is not in the bot emojis servers.");
				} else {
					success("Success! The bot is in the bot emojis servers.");
				}
				res();
			}).catch(() => {
	...
	() => {
		...
		return new Promise((res) => {
			...
			client.connect().then(async () => {
				success("Success! The bot should be able to connect to Mongo database!");
				res();
			}).catch(() => {
				error("Error, please verify if the MongoDB server is started.");
				res();
			});
		});
	},
...
(async () => {
	...
	console.log(chalk.yellow("\n\nThank you for using TruDetto Utilities. If you need more help, join our support server here: https://discord.gg/Xxxq68DngY"));
	process.exit(0);
})();
